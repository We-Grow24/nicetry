...
export default function UserDropdown() {
  ...
  useEffect(() => {
    ...
    async function loadUser() {
      ...
      const { data } = await supabase
        ...
        .single();
      if (mounted) {
        setUserRow(data ?? { email: user.email ?? null, credits: 0 });
      }
    }
    loadUser();
  ...
  const displayEmail = userRow?.email ?? "";
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {/* Credits badge */}
        {userRow !== null && (
          <span className="hidden sm:inline-flex items-center gap-1 rounded-full bg-indigo-50 dark:bg-indigo-900/40 border border-indigo-200 dark:border-indigo-700 px-2 py-0.5 text-xs font-semibold text-indigo-600 dark:text-indigo-300 mr-1">
      ...
      >
        ...
        <div className="pb-3 border-b border-gray-200 dark:border-gray-800">
          ...
          <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-indigo-50 dark:bg-indigo-900/40 border border-indigo-200 dark:border-indigo-700 px-2.5 py-1 text-xs font-semibold text-indigo-600 dark:text-indigo-300">
            <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
            </svg>
            {credits} credits
